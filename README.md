...
- 各 leg：`scheme:"exact"`, 金額 `"10000"`（atomic USDC 6桁）, `asset`(Solana USDC mint), `payTo`, `extra:{resource, feePayer}`。**金額フィールド名は version で異なる**（値は同一）：**v1 leg=`maxAmountRequired`**（正典 `x402@1.2.0` の PaymentRequirementsSchema で確認）、**v2 leg=`amount`**（§6 OSD本番実測）。
